...
class GraphTemporalKANv3(nn.Module):
    """
    Temporal sequence model:
    - encoder-only graph backbone
    - transformer over graph embeddings
    - CLS + attention pool + mean pool + last token fusion
    - final KAN head for sequence prediction
    - auxiliary linear head on the last graph embedding (training only)
    """
    ...
    def __init__(
        ...
        self.aux_last_head = nn.Sequential(
            nn.LayerNorm(graph_emb_dim),
    ...
    def forward(self, batched_graph_list, update_grid=False, return_aux=False):
        ...
        for batch_t in batched_graph_list:
            g = self.graph_encoder(batch_t)
            seq_embs.append(g)

        token_graphs = torch.stack(seq_embs, dim=1)  # [B, L, D]
        last_graph_embedding = token_graphs[:, -1, :]
        aux_last_logits = self.aux_last_head(last_graph_embedding)
        ...
        if return_aux:
            return logits, {
                ...
                "last_graph_embedding": last_graph_embedding,
                "aux_last_logits": aux_last_logits,
                "cls_token": cls_token,
                "attn_pool": attn_pool,
                "mean_pool": mean_pool,
