...
# ============================================================
# GraphAttentionKAN (multitask, compact KAN head)
# ============================================================
class GraphAttentionKAN(nn.Module):
    """
    Multi-relation Graph Attention Encoder + compact KAN graph head
    with optional node-classification head for multitask learning.

    Main difference from the original version:
    - Keep KAN-FFN in every block.
    - Reduce graph-head parameters by inserting a lightweight bottleneck
      before the KAN head.

    Example with hidden_dim=128:
      readout dim = 256
      original head: KAN([256, 64, num_classes])
      compact head : Linear(256->32) + KAN([32, 32, num_classes])

    This preserves a KAN-based decision layer while making the head much smaller.
    """
    ...
    def __init__(
        ...
        # Lightweight pre-projection to reduce KAN head size
        self.head_preproj = nn.Sequential(
            nn.LayerNorm(readout_dim),
            nn.Linear(readout_dim, head_bottleneck_dim),
            nn.SiLU(),
            nn.Dropout(dropout),
        )

        # graph/window head (smaller KAN)
        self.head = KAN(
            layers_hidden=[head_bottleneck_dim, kan_hidden, num_classes],
    ...
    def classify_graph(self, g, update_grid=False):
        g_small = self.head_preproj(g)
        return self.head(g_small, update_grid=update_grid)
    ...
    def forward(
        ...
        if return_graph_embedding and return_node_logits:
            return {
                ...
                "graph_embedding": g,
                "graph_embedding_prekan": self.head_preproj(g),
