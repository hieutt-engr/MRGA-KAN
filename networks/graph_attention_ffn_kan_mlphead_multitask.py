...
# FFN-KAN backbone + MLP graph head + MLP node head
# ============================================================
class GraphAttentionKAN(nn.Module):
    """
    Multi-relation Graph Attention Encoder with KAN-FFN backbone.

    Variant in this file:
    - FFN inside each block: KAN
    - graph/window head: MLP (NOT KAN)
    - node head: MLP
    """
    ...
    def __init__(
        ...
        # Graph head hidden
        kan_hidden=128,
        # accepted for CLI compatibility, unused for MLP head
        kan_grid_size=5,
        kan_spline_order=3,
        kan_scale_noise=0.1,
        kan_scale_base=1.0,
